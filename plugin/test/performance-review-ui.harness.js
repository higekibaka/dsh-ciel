...
export function fixtureClientRequire(React, modules = {}) {
  const primitives = {
    ...
    // Interface-only Button fixture so the native adapter path is exercised;
    // it forwards every prop (variant/size/data-ciel-*) like the real primitive.
    Button: ({ children, ...props }) => React.createElement('button', { ...props, 'data-fixture-native': 'Button' }, children),
  }
  return name => {
