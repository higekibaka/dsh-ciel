...
| `advise-command/` | advcmd-1 | pkg-1（host+client） | **P3 原型，待用户实测确认**——确认后静态化进 bundle；重启后把 host.js/client.js 分别作为 `code.host`/`code.client` 重新 define（idPrefix advcmd）+ run |
