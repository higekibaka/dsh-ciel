...
- **修复回路**：主模型看不到评审，用户是批评者发现 → 作者修复之间的过滤器；「回传主模型」按钮（用户勾选制）——勾选行为同时提供注意力投放质量的地面真值，成为调 prompt/证据面的评估回路。**已落地 0.7.0**（annfbk 动态原型三项实测确认后静态化：送达唤醒 ✓、定位信息 ✓、去重跳过 ✓）：`advisorReview` Remote 加 `feedback` 方法（`agent.followup` 次轮语义，绝不打断在跑 turn），去重账本按 `reviewId#index` 键、WAL 落 `$DSH_HOME/dsh-advisor/feedback/<sessionId>.jsonl`（直接 node:fs，无动态插件沙箱问题）；客户端勾选 UI 直接进 `buildPanel`——真实 reviewId 取自 entry 本身（原型的 title 匹配猜测链删除），勾选/已回传/注记全部 store 化随重绘保留，`list` 顺带下发 `sentKeys` 让刷新后已回传条目仍置灰。
