...
- [x] 只读工具白名单与预算硬上限的**真实例**执行证据（advisor-test 实拍，
  ...
  疑点场景也自我压缩到 1 次调用）
- [x] 条件风险措辞在可证伪场景下绝迹（升级为证据引用）——A/B 语料全量
  实证（`docs/ab/ab-flash-v3.md`）：S2 两条 blocker、S5 一条 nit 均带
  行号级引用，「若…请先核实」式条件推诿在证伪路径上绝迹。契约 v3.2：
  探索支撑的批注无论级别都必须带 evidence（citation 是证据不是级别
  徽章）。已知残留风险：工具误用可致「证据为真、结论为假」（实拍
  `glob "./*.js"` 模式误用一例）——evidence 引用设计恰好让此类误判
  可被人一眼识破，A/B 已纳入「工具误用率」维度
